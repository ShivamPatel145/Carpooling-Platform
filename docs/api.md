...
> Adding a new resource follows the fixed shape above: `route.ts` (+ `[id]/`, `my/`, `stats/` as
> needed), each handler wrapped in `withErrorHandler`, opening with `requirePermission`, scoped by
> `orgId`. See the `generic-crud` skill and `features/vehicle/` as the reference.
