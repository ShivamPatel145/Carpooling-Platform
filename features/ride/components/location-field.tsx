 ...
 * geocoder — instead the user's SAVED PLACES (Home/Office/…, Slice B's table) listed first, then a
 * curated landmark list (so the demo "just works"), plus manual lat/lng entry. Writes a GeoPoint
 * ({ label, lat, lng }) into an RHF field. OSRM turns the two points into a real road route
 * server-side (see /api/ride).
 *
 * Swap the landmark list for a Nominatim/Mapbox autocomplete post-hackathon — the field's output
...
export function LocationField<T extends FieldValues>({
  ...
  // Saved-place autofill (Home/Office/…). Numeric columns arrive as strings — coerce.
  const { data: savedPlaces } = useSavedPlaces();
  const saved: GeoPoint[] = React.useMemo(
    () => (savedPlaces ?? []).map((p) => ({ label: p.label, lat: Number(p.lat), lng: Number(p.lng) })),
    [savedPlaces],
  );
            ...
            <>
              ...
              <div className="grid gap-0.5 sm:grid-cols-2">
                {saved.map((p) => (
                  ...
                  >
                    <Star className="h-3.5 w-3.5 text-[color:var(--amber-strong)]" />
                    {p.label}
                  </button>
                ))}
              </div>
              <p className="mt-1 border-t px-2 pt-1 text-[11px] font-medium uppercase tracking-wide text-muted-foreground">
                Landmarks
              </p>
            </>
          )}
